(function() {
    ...
    */
    function createExpandedElement(node) {
        ...
        const caretElem     = createElement('div', { className: 'caret-icon', children: [iElem], })
        const handleClick   = node.toggle.bind(node)
        caretElem.addEventListener('click', handleClick)

        const indexElem     = createElement('div', { className: 'json-index',   content: node.key, })
        const typeElem      = createElement('div', { className: 'json-type',    content: node.type, })
        const keyElem       = createElement('div', { className: 'json-key',     content: node.key, })
        ...
        const lineElem = createElement('div', { className: 'line', children: lineChildren })
        if (node.depth > 0) {
    ...
    */
    function createNotExpandedElement(node) {
        ...
        createElement('div', {
            ...
            content: isivaluecontent()
        })
        function isivaluecontent() {
            if (valueContent == 'true') {
                return 'true'
            } else if (valueContent == 'false') {
                return 'false'
        ...
        const $children = [caretElem, keyElem, separatorElement, valueElement]
        const lineElem  = createElement('div', { className: 'line', children: $children })
        if (node.depth > 0) {
            lineElem.style = 'margin-left: ' + node.depth * 24 + 'px;'
        }
        return lineElem
